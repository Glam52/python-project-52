...
class UserTests(TestCase):
    ...
    def setUp(self):
        self.user_data = {
            ...
            'first_name': 'Test',
            'last_name': 'User',
            'password': 'SecurePassword123',  # Используем только один параметр для пароля
        }
    ...
    def test_user_create(self):
        def test_user_create(self):
            ...
            self.assertEqual(response.status_code, 302)  # Проверка на перенаправление
            self.assertTrue(User.objects.filter(username='testuser').exists())  # Проверка, что пользователь создан

    def test_user_update(self):
        updated_data = {'first_name': 'Updated', 'last_name': 'User', 'password1': 'NewPassword123', 'password_confirm': 'NewPassword123'}
